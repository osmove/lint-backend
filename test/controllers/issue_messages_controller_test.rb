...
class IssueMessagesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create issue_message' do
    assert_difference('IssueMessage.count') do
      post issue_messages_url,
           params: { issue_message: { body: @issue_message.body, issue_id: @issue_message.issue_id,
                                      repository_id: @issue_message.repository_id, slug: @issue_message.slug, title: @issue_message.title, user_id: @issue_message.user_id, username: @issue_message.username } }
    end
  ...
  test 'should update issue_message' do
    patch issue_message_url(@issue_message),
          params: { issue_message: { body: @issue_message.body, issue_id: @issue_message.issue_id,
                                     repository_id: @issue_message.repository_id, slug: @issue_message.slug, title: @issue_message.title, user_id: @issue_message.user_id, username: @issue_message.username } }
