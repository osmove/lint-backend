      ...
      if @user.persisted? || @user.save
        ...
        if repositories.any?
          repositories.each do |repo|
            next unless Repository.where(user: @user, web_address: repo['html_url'],
                                         git_address: repo['git_url']).blank? && (repo['owner']['login'] == @user.login)

            @platform = if repo['language'].present?
                          begin
                            Platform.where(slug: repo['language'].downcase).first
                          rescue StandardError
                            nil
                          end
                        else
                          begin
                            Platform.where(name: 'Other').first
                          rescue StandardError
                            nil
                          end
                        end
          ...
          if organizations.any?
            organizations.each do |org|
              ...
              if repositories.any?
                repositories.each do |repo|
                  next if Repository.where(user_id: @organization, slug: repo['name'], web_address: repo['html_url'],
                                           git_address: repo['git_url']).present?
              ...
              if @organization.save!
                ...
                if members.any?
                  members.each do |member|
                    ...
                    if @organization_member.present?
                      ...
                      Rails.logger.debug(@member.errors.full_messages) unless @member.save!
                    end
                  end
                end
                ...
                unless teams_json.nil?
                  ...
                  # puts(teams)
                  teams.each do |team|
                    next if Team.where(name: team['name'], user: @organization).present?
                      ...
                      if members.any?
                        members.each do |member|
                          ...
                          if @team_membership.present?
                            ...
                            Rails.logger.debug(@membership.errors.full_messages) unless @membership.save!
                          end
                        end
                      end
                    else
                ...
                Rails.logger.debug(@organization.errors.full_messages)
