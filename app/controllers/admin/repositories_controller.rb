module Admin
  class RepositoriesController < Admin::BaseController
    ...
    def index
      @repositories = Repository.order(created_at: :desc).page(params[:page]).per(15)
    end
  ...
  private
    ...
    def set_repository
      ...
      raise ActionController::RoutingError, 'Repository Not Found' if @repository.blank?
    ...
    def repository_params
      params.expect(repository: %i[name slug user_id status deploy_to server_size domain_slug geo_zone hosting_plan_id type platform_id has_encryption has_deployment requires_two_step_authentication policy_id git_host
