...
module Admin
        ...
        unless files_json.nil?
          ...
          files.each do |file|
            if Document.where(repository: @repository, name: file['name'], path: file['path']).blank?
              ...
              Rails.logger.debug(@file.errors.full_messages) unless @file.save!
            else
      ...
      if @document.is_folder?
        ...
        files_content.each do |content|
          ...
          next unless Document.where(name: content['name'], document: @document, path: content['path'], is_folder: @is_folder, repository:
            @document.repository, size: content['size'], extension: content['name'].partition('.').last, type: @file_type).none?
          ...
          Rails.logger.debug(@content.errors.full_messages) unless @content.save!
        end
  ...
  private
    ...
    def document_params
      params.expect(document: %i[name path is_folder size extension content repository document])
    end
  end
end
